...
create or replace function public.record_inventory_movement(
  p_inventory_id uuid, p_movement_type text, p_quantity integer,
  p_reference text default null, p_notes text default null
) returns public.inventory_movements
language plpgsql security definer set search_path = public
as $$
declare v_inventory public.inventory; v_movement public.inventory_movements; v_next_stock integer;
begin
  ...
  update public.inventory set stock_quantity = v_next_stock, updated_at = now() where id = p_inventory_id;
  insert into public.inventory_movements (variant_id, movement_type, quantity, reference, notes, created_by)
  values (v_inventory.variant_id, p_movement_type, p_quantity, p_reference, p_notes, auth.uid()) returning * into v_movement;
  return v_movement;
end;
$$;
...
create table if not exists public.user_addresses (
  ...
  label text not null default 'Home', full_name text not null, line1 text not null,
  city text not null, zip text not null, phone text not null default '',
...
create table if not exists public.user_payment_methods (
  ...
  label text not null, detail text not null default '', is_default boolean not null default false,
